...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/admin/data?collection=billing_and_invoices")
      .then(r => r.json())
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingBottom: "1rem", borderBottom: "1px solid rgba(107,142,35,0.15)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
          <div style={{ width: 40, height: 40, borderRadius: 4, background: "rgba(76,201,240,0.12)", border: "1px solid rgba(76,201,240,0.3)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.2rem", color: "#4cc9f0", boxShadow: "0 0 12px rgba(76,201,240,0.12)" }}>
            ◇
          </div>
          <div>
            <h1 style={{ fontFamily: "'Orbitron', sans-serif", fontSize: "0.95rem", fontWeight: 700, color: "#fff", letterSpacing: "2px", textTransform: "uppercase" }}>Billing and Invoices</h1>
            <p style={{ fontSize: "0.75rem", color: "#94a3b8", marginTop: 2 }}>Invoice Management & Payments</p>
          </div>
        </div>
        <div style={{ display: "flex", gap: "0.75rem", alignItems: "center" }}>
          ...
          <button onClick={() => { setForm({}); setShowForm(true); }} style={{ background: "rgba(76,201,240,0.12)", border: "1px solid rgba(76,201,240,0.3)", borderRadius: 2, color: "#fff", padding: "0.45rem 1rem", fontFamily: "'Orbitron', sans-serif", fontSize: "0.65rem", fontWeight: 700, letterSpacing: "1.5px", cursor: "pointer", textTransform: "uppercase" }}>+ NEW</button>
        </div>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "1rem" }}>
        {[
          { label: "TOTAL", value: loading ? "—" : data.length, color: "#4cc9f0" },
      ...
      <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(107,142,35,0.15)", borderRadius: 4, overflow: "hidden" }}>
        <div style={{ padding: "0.75rem 1.25rem", borderBottom: "1px solid rgba(107,142,35,0.15)", display: "flex", alignItems: "center", gap: "0.5rem" }}>
          ...
          <span style={{ fontFamily: "'Orbitron', sans-serif", fontSize: "0.7rem", color: "#fff", letterSpacing: "1px", textTransform: "uppercase" }}>Billing and Invoices REGISTER</span>
              ...
              <thead>
                <tr style={{ background: "rgba(107,142,35,0.05)", borderBottom: "1px solid rgba(107,142,35,0.15)" }}>
                  {Object.keys(filtered[0] || {}).filter(k => !["id","createdAt","updatedAt","passwordHash"].includes(k)).slice(0,6).map(k => (
                    <th key={k} style={{ padding: "0.65rem 1rem", textAlign: "left", fontFamily: "'Orbitron', sans-serif", fontSize: "0.6rem", color: "#4cc9f0", letterSpacing: "1.5px", textTransform: "uppercase" }}>{k}</th>
                  ))}
                  <th style={{ padding: "0.65rem 1rem", fontFamily: "'Orbitron', sans-serif", fontSize: "0.6rem", color: "#4cc9f0", letterSpacing: "1.5px" }}>ACTIONS</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map((row, i) => (
                  <tr key={row.id || i} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)", transition: "background 0.2s" }} onMouseEnter={e => e.currentTarget.style.background="rgba(255,255,255,0.02)"} onMouseLeave={e => e.currentTarget.style.background="transparent"}>
                    ...
                    <td style={{ padding: "0.65rem 1rem" }}>
                      <button onClick={() => { setForm(row); setShowForm(true); }} style={{ background: "rgba(107,142,35,0.08)", border: "1px solid rgba(107,142,35,0.2)", borderRadius: 2, color: "#4cc9f0", padding: "0.3rem 0.6rem", fontFamily: "'Orbitron', sans-serif", fontSize: "0.55rem", cursor: "pointer", letterSpacing: "1px" }}>EDIT</button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {showForm && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", backdropFilter: "blur(4px)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setShowForm(false)}>
          <div style={{ background: "rgba(13,17,23,0.98)", border: "1px solid rgba(76,201,240,0.3)", borderRadius: 4, padding: "2rem", width: "100%", maxWidth: 520, position: "relative" }} onClick={e => e.stopPropagation()}>
            <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 2, background: "linear-gradient(90deg, transparent, #4cc9f0, transparent)" }} />
            ...
            <div style={{ display: "flex", gap: "0.75rem", justifyContent: "flex-end", marginTop: "1.5rem" }}>
              <button onClick={() => setShowForm(false)} style={{ background: "transparent", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 2, color: "#94a3b8", padding: "0.5rem 1rem", fontFamily: "'Orbitron', sans-serif", fontSize: "0.65rem", cursor: "pointer", letterSpacing: "1px" }}>CANCEL</button>
                  <button disabled={saving} onClick={async () => {
                    setSaving(true);
                    try {
                      const url = form.id ? `/api/admin/data?collection=billing_and_invoices&id=${form.id}` : `/api/admin/data?collection=billing_and_invoices`;
                      const method = form.id ? "PUT" : "POST";
                      const res = await fetch(url, { method, headers: {"Content-Type":"application/json"}, body: JSON.stringify(form) });
                      if (res.ok) {
                        const updated = await fetch("/api/admin/data?collection=billing_and_invoices").then(r=>r.json());
