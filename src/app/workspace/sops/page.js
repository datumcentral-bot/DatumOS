...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/admin/data?collection=sop_how-to")
      .then(r => r.json())
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingBottom: "1rem", borderBottom: "1px solid rgba(107,142,35,0.15)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
          ...
          <div>
            <h1 style={{ fontFamily: "'Orbitron', sans-serif", fontSize: "0.95rem", fontWeight: 700, color: "#fff", letterSpacing: "2px", textTransform: "uppercase" }}>SOP How-To</h1>
            <p style={{ fontSize: "0.75rem", color: "#94a3b8", marginTop: 2 }}>Standard Operating Procedures</p>
          </div>
        </div>
      ...
      <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(107,142,35,0.15)", borderRadius: 4, overflow: "hidden" }}>
        <div style={{ padding: "0.75rem 1.25rem", borderBottom: "1px solid rgba(107,142,35,0.15)", display: "flex", alignItems: "center", gap: "0.5rem" }}>
          ...
          <span style={{ fontFamily: "'Orbitron', sans-serif", fontSize: "0.7rem", color: "#fff", letterSpacing: "1px", textTransform: "uppercase" }}>SOP How-To REGISTER</span>
          ...
          <div style={{ background: "rgba(13,17,23,0.98)", border: "1px solid rgba(107,142,35,0.3)", borderRadius: 4, padding: "2rem", width: "100%", maxWidth: 520, position: "relative" }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", gap: "0.75rem", justifyContent: "flex-end", marginTop: "1.5rem" }}>
              ...
              <button disabled={saving} onClick={async () => {
                setSaving(true);
                try {
                  const url = form.id ? `/api/admin/data?collection=sop_how-to&id=${form.id}` : `/api/admin/data?collection=sop_how-to`;
                  const method = form.id ? "PUT" : "POST";
                  const res = await fetch(url, { method, headers: {"Content-Type":"application/json"}, body: JSON.stringify(form) });
                  if (res.ok) {
                    const updated = await fetch("/api/admin/data?collection=sop_how-to").then(r=>r.json());
