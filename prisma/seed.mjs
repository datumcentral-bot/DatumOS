  ...
  for (const p of projectData) {
    const created = await prisma.project.create({
      data: {
        code: p.code,
        name: p.name,
        client: { connect: { id: clients[p.client].id } },
        status: "ACTIVE",
        health: p.health,
    ...
    for (const dc of p.divs) {
      await prisma.projectDivision.create({
        data: { projectId: created.id, divisionId: divisions[dc].id },
      });
    }
    ...
    for (const m of ms) {
      await prisma.milestone.create({
        data: { projectId: created.id, title: m.title, status: m.status, dueDate: daysAhead(m.d < 0 ? -Math.abs(m.d) : m.d) },
      });
    }
  }
  ...
  for (const a of assignPlan) {
    for (const [email, role, pct] of a.people) {
      await prisma.projectAssignment.create({
        data: { projectId: projects[a.code].id, userId: users[email].id, role: role, isLead: role === "Lead BIM Manager" },
      });
    }
  }
